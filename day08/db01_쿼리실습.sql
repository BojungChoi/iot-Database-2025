...
-- 조건부 논리 표현식 제어 : CASE
/* 샘플문제 1 - 
신규프로젝트 성공으로 급여인상이 결정됨. 
사원 현재 107명 19개 업무에 소속되어 근무중
급여인상 대상자는 업무(Distinct job_id) 중 다음 5개 업무에서 일하는 사원에 해당
나머지는 동결 ㅠㅠ 5개업무의 급여 인상안은 다음과 같음(107행)
HR_REP(10%), MK_PER(12%), PR_REP(15%), SA_REP(18%), IT_PROG(20%)
*/
select employee_id
	 , concat(first_name,' ',last_name) as 'name'
     , job_id
     , salary
     , CASE job_id when 'HR_REP' then 1.1 * salary
				   when 'MK_REP' then 1.12 * salary
                   when 'PR_REP' then 1.15 * salary
                   when 'SA_REP' then 1.18 * salary
                   when 'IT_PROG' then 1.2 * salary
	   else salary
        end "New Salary"
from employees;
...
*/
-- 형변환 함수 cast(), convert()
select cast('09' as UNSIGNED ); -- cast는 as 
select convert('09', SIGNED); -- covert 는 ,
...
/*
Error Code: 1055. Expression #2 of SELECT list is not in GROUP BY clause and contains nonaggregated column 'hr.employees.hire_date' 
which is not functionally dependent on columns in GROUP BY clause; this is incompatible with sql_mode=only_full_group_by
*/

-- ROLLUP
/* 샘플 - 부서와 업무별 급여합계를 구하고 신년도 급여수준 레벨을 지정하려고 함.
			부서 번호와 업무를 기준으로 전체 행을 그룹별로 나누어 급여합계와 인원수를 출력(20개행)
*/
...
/* 문제 1 - 이전문제를 활용, 집계결과가 아니면 (ALL-DEPTs)라고 출력, 업무에 대한 집계결과가 아니면(ALL-JOBs)를 출력
			rollup으로 만들어진 소계면 (ALL-JOBs), 총계면 (ALL-DEPTs)
*/
 select case grouping(department_id) when 1 then'(ALL-DEPRs)' else ifnull(department_id, '부서없음') end 'Dept#',
...
select department_id
     ...
     , ntile(12) over (order by sum(salary) desc) as 'Bucket#' -- 범위별로 등급을 매기는 키워드
  from employees
 group by department_id;
 
 /* 문제 1 - 부서별 급여를 기준으로 내림차순 정렬하시오. 이때 다음 세가지 함수를 이용하여, 순위를 출력하시오(107행)
 */
 select employee_id
	  , last_name
      , salary
      , department_id
      , rank() over (partition by department_id order by salary desc) as 'Rank' -- 1, 1, 3 순위 매기기 rank 
      , dense_rank() over (partition by department_id order by salary desc) as 'Dense Rank' -- 1, 1, 2 순위매기기 dense_rank
