...
# 5. DB처리 함수들 정의
## 5-1. showDatas() - DB 학생정보 테이블에 데이터를 가져와서 TreeView에 표시
def showDatas():
    '''
    ...
    global dataView     # 외부에있는 변수를 전역변수로 사용 선언
    ### 5-1-1 DB 연결. 커넥션 객체생성 -> 커서 -> 쿼리실행 -> 커서로 데이터 패치 -> 커서 종료 -> 커넥션 종료
    ...
    data = cursor.fetchall()        # 쿼리 실행데이터 전부 가져오기

    ### 5-1-3 가져온 데이터 트리뷰에 추가
    print(data)
    
    ### 5-2-1
    dataView.delete(*dataView.get_children())                                           # 최초, 중간에 showDatas() 호출시 마다 트리뷰 클리어
    for i, (std_id, std_name, std_mobile, std_regyear) in enumerate(data,start=1):      # enumerate는 인덱스와 아이템을 동시접근
        dataView.insert('','end',values=(std_id, std_name, std_mobile, std_regyear))    # 트리뷰 마지막에 'end' 추가

    cursor.close()      # 커서 종료
    conn.close()        # 커넥션 종료

# 6. getData() - 트리뷰 한줄 더블클릭한 값 엔트리에 표시
def getData(event):
    '''
    ...
    ent3.delete(0, END) # 번호 기존 데이터 삭제
    ent4.delete(0, END) # 입학년도 기존 데이터 삭제

    ## 6-2 트리뷰 선택항목 ID 가져오기('I001')
...
def addData():
    ...
    try:
        ...
        conn.commit() # 트랜잭션 확정
        lastid = cursor.lastrowid # 마지막에 insert 된 레코드 id를 가져옴(auto_increment)
        print(lastid)
...
def modData():
    ...
    if std_id == '':
        messagebox.showerror('UPDATE','수정할 데이터를 선택하세요')
        return
...
def delData():
    ...
    std_id = ent1.get()         # 학생번호
    if std_id == '':
        messagebox.showerror('DELETE','삭제 데이터를 선택하세요')
        return    
...
ent1.config(state='readonly', foreground='blue', disabledbackground='blue')   # 학생번호는 값을 입력못하게 방지(입력 칸 회색으로바뀜)
...
tk.Button(root, text='추가', font=myFont, height=1, width=10 , bg = 'pink', command=addData).place(x=50, y=140)    # 추가버튼
tk.Button(root, text='수정', font=myFont, height=1, width=10 , bg = 'pink', command=modData).place(x=150, y=140)   # 수정버튼
tk.Button(root, text='삭제', font=myFont, height=1, width=10 , bg = 'pink', command=delData).place(x=250, y=140)   # 삭제버튼

## 8-1 수전버튼에 modData() 함수를 연결



## 4-4 트리 뷰 (행과 열로 만들어진 데이터 표현할 때 좋은 위젯)
cols = ('학생번호', '학생명', '핸드폰', '입학년도')
dataView = ttk.Treeview(root, columns=cols, show='headings', height= 14)

## 4-5 트리뷰 설정
for col in cols:
    dataView.heading(col, text=col)                 # 각열 제목을 cols 변수 하나씩 지정
...
# 6-2. 트리뷰항목 더블클릭하면 이벤트 발생(getData() 함수 호출)
